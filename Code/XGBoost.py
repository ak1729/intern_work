...
xgb_model.fit(F, X)  # Predicting the original higher-dimensional representation
predictions = xgb_model.predict(F)
end=time.time()

tot=end-start

localXgboost = localProcrustes(X, predictions)
mse = mean_squared_error(predictions, X)

print("Local Proscrustes Measure: ", localXgboost)
